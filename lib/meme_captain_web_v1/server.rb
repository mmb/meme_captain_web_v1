...
module MemeCaptainWebV1
  ...
  class Server < Sinatra::Base
    ...
    def gen(p)
      logger.debug "params:\n#{MemeCaptainWebV1.pretty_format(p)}"
      norm_params = normalize_params(p)
      logger.debug 'normalized params:'
      logger.debug(MemeCaptainWebV1.pretty_format(norm_params))
      ...
      if existing = MemeData.first(
        ...
        logger.debug(MemeCaptainWebV1.pretty_format(existing))
        existing
      else
        if same_source = MemeData.find_by_source_url(norm_params.u)
          ...
          logger.debug MemeCaptainWebV1.pretty_format(upload)
          ...
          if source_fetch_fail = SourceFetchFail.find_by_url(norm_params.u)
            ...
            logger.debug(MemeCaptainWebV1.pretty_format(source_fetch_fail))
            ...
            begin
              ...
              logger.debug(MemeCaptainWebV1.pretty_format(new_source_fetch_fail))
        ...
        open(source_fs_path, 'rb') do |source_io|
          t1 = MemeCaptain::TextPos.new(norm_params.t1, norm_params.t1x, norm_params.t1y,
            norm_params.t1w, norm_params.t1h)

          t2 = MemeCaptain::TextPos.new(norm_params.t2, norm_params.t2x, norm_params.t2y,
          ...
          logger.debug "meme data:\n#{MemeCaptainWebV1.pretty_format(meme_data)}"
    ...
    get '/g' do
      begin
        ...
        logger.error(MemeCaptainWebV1.pretty_format(error.backtrace))
